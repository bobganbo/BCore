...
class LibRedis{
    ...
    public static $linkHandle = array();//连接句柄
    ...
    private function initRedis($conf){
        ...
        foreach ($conf['master'] as $k=>$v){
            ...
            if($obj->pconnect($v['host'],$v['port'])){
                $obj->auth($v['auth']);
                self::$linkHandle['master'][] = $obj;//主连接句柄
            }
        }
        //slave对象创建，可以多个
        foreach ($conf['slave'] as $k=>$v){
            ...
            if($obj->pconnect($v['host'],$v['port'])){
                $obj->auth($v['auth']);
                self::$linkHandle['slave'][] = $obj;//从连接句柄
            }
        }
    }

    /**
     * 获得redis Resources,随机获取redis对象
     *
    ...
    public function getRedis($key=null,$tag='master'){
        ...
        $arr_index = $this->getHostByHash($key,count(self::$linkHandle[$tag])); //获得相应主机的数组下标,根据键值，hash到不同的主机
    ...
    private function getHostByHash($key,$n){
        if($n<2) return 0;
        $u = strtolower($key);
        $id = sprintf("%u", crc32($key));//计算字符串的 32 位 CRC（循环冗余校验）
     ...
     * @param $key
     * @param $value
     * @param $exp 过期时间,0表示永不过期
     */
    public function set($key,$value,$exp=0){
        $redis = $this->getRedis($key);
        $redis->set($key,$value);
        !empty($exp) && $redis->expire($key,$exp);//设置键值过期时间
    }
    /**
